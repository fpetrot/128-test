...
def run_test(test_filename: str, silent: bool = True) -> TestStatus:
    ...
    qemu = subprocess.Popen([QEMU_EXEC, '-machine', 'virt', '-cpu', 'x-rv128,zicsr=on,zicbom=on,zicboz=on,zba=on,zbb=on,zbs=on', '-accel', 'tcg,thread=single', '-bios', 'none', '-nographic', '-kernel', f'out/{test_name}', '-S', '-gdb', 'tcp::1144'], stdout=(open("/dev/null") if silent else sys.stdout))
