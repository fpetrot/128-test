...
def run_test(test_filename: str, silent: bool = True) -> TestStatus:
    ...
    qemuproc = subprocess.Popen([QEMU_EXEC, '-machine', 'virt', '-cpu', 'x-rv128', '-accel', 'tcg,thread=single', '-bios', 'none', '-nographic',
        '-kernel', f'out/{test_name}', '-S', '-gdb', 'tcp::1244'], stdout=(open("/dev/null") if silent else sys.stdout))
