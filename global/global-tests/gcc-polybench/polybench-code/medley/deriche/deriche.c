/* deriche.c: this file is part of PolyBench/C */

typedef unsigned int wint_t;

#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include <math.h>
#include <stdlib.h>
#include "deriche.h"


/* Array initialization. */
static
void init_array (int w, int h, DATA_TYPE* alpha,
		 DATA_TYPE** imgIn,
		 DATA_TYPE** imgOut)
{
  int i, j;

  *alpha=0.25; //parameter of the filter

  //input should be between 0 and 1 (grayscale image pixel)
  for (i = 0; i < w; i++)
     for (j = 0; j < h; j++)
	imgIn[i][j] = (DATA_TYPE) ((313*i+991*j)%65536) / 65535.0f;
}


/* DCE code. Must scan the entire live-out data.
   Can be used also to check the correctness of the output. */
static
void print_array(int w, int h,
		 DATA_TYPE** imgOut)
{
  int i, j;

  for (i = 0; i < w; i++){
    for (j = 0; j < h; j++) {
            printf("%0.2f ", imgOut[i][j]);
    }
    printf("\n");
  }
}



/* Main computational kernel. The whole function will be timed,
   including the call and return. */
/* Original code provided by Gael Deest */
static 
void kernel_deriche(int w, int h, DATA_TYPE alpha,
       DATA_TYPE** imgIn,
       DATA_TYPE** imgOut,
       DATA_TYPE** y1,
       DATA_TYPE** y2) {
    int i,j;
    DATA_TYPE xm1, tm1, ym1, ym2;
    DATA_TYPE xp1, xp2;
    DATA_TYPE tp1, tp2;
    DATA_TYPE yp1, yp2;
   
    DATA_TYPE k;
    DATA_TYPE a1, a2, a3, a4, a5, a6, a7, a8;
    DATA_TYPE b1, b2, c1, c2;

#pragma scop
   k = (SCALAR_VAL(1.0)-EXP_FUN(-alpha))*(SCALAR_VAL(1.0)-EXP_FUN(-alpha))/(SCALAR_VAL(1.0)+SCALAR_VAL(2.0)*alpha*EXP_FUN(-alpha)-EXP_FUN(SCALAR_VAL(2.0)*alpha));
   a1 = a5 = k;
   a2 = a6 = k*EXP_FUN(-alpha)*(alpha-SCALAR_VAL(1.0));
   a3 = a7 = k*EXP_FUN(-alpha)*(alpha+SCALAR_VAL(1.0));
   a4 = a8 = -k*EXP_FUN(SCALAR_VAL(-2.0)*alpha);
   b1 =  POW_FUN(SCALAR_VAL(2.0),-alpha);
   b2 = -EXP_FUN(SCALAR_VAL(-2.0)*alpha);
   c1 = c2 = 1;

   for (i=0; i< w; i++) {
        ym1 = SCALAR_VAL(0.0);
        ym2 = SCALAR_VAL(0.0);
        xm1 = SCALAR_VAL(0.0);
        for (j=0; j<h; j++) {
            y1[i][j] = a1*imgIn[i][j] + a2*xm1 + b1*ym1 + b2*ym2;
            xm1 = imgIn[i][j];
            ym2 = ym1;
            ym1 = y1[i][j];
        }
    }

    for (i=0; i<w; i++) {
        yp1 = SCALAR_VAL(0.0);
        yp2 = SCALAR_VAL(0.0);
        xp1 = SCALAR_VAL(0.0);
        xp2 = SCALAR_VAL(0.0);
        for (j=h-1; j>=0; j--) {
            y2[i][j] = a3*xp1 + a4*xp2 + b1*yp1 + b2*yp2;
            xp2 = xp1;
            xp1 = imgIn[i][j];
            yp2 = yp1;
            yp1 = y2[i][j];
        }
    }

    for (i=0; i<w; i++)
        for (j=0; j<h; j++) {
            imgOut[i][j] = c1 * (y1[i][j] + y2[i][j]);
        }

    for (j=0; j<h; j++) {
        tm1 = SCALAR_VAL(0.0);
        ym1 = SCALAR_VAL(0.0);
        ym2 = SCALAR_VAL(0.0);
        for (i=0; i<w; i++) {
            y1[i][j] = a5*imgOut[i][j] + a6*tm1 + b1*ym1 + b2*ym2;
            tm1 = imgOut[i][j];
            ym2 = ym1;
            ym1 = y1 [i][j];
        }
    }
    

    for (j=0; j<h; j++) {
        tp1 = SCALAR_VAL(0.0);
        tp2 = SCALAR_VAL(0.0);
        yp1 = SCALAR_VAL(0.0);
        yp2 = SCALAR_VAL(0.0);
        for (i=w-1; i>=0; i--) {
            y2[i][j] = a7*tp1 + a8*tp2 + b1*yp1 + b2*yp2;
            tp2 = tp1;
            tp1 = imgOut[i][j];
            yp2 = yp1;
            yp1 = y2[i][j];
        }
    }

    for (i=0; i<w; i++)
        for (j=0; j<h; j++)
            imgOut[i][j] = c2*(y1[i][j] + y2[i][j]);

#pragma endscop
}


int main(int argc, char** argv)
{
  /* Retrieve problem size. */
  int w = W;
  int h = H;

  /* Variable declaration/allocation. */
  DATA_TYPE alpha;
  DATA_TYPE** imgIn;
  imgIn = malloc(w * sizeof(DATA_TYPE*));
  for (int i = 0; i < w; i++) {
    imgIn[i] = malloc(h * sizeof(DATA_TYPE));
  }
  DATA_TYPE** imgOut;
  imgOut = malloc(w * sizeof(DATA_TYPE*));
  for (int i = 0; i < w; i++) {
    imgOut[i] = malloc(h * sizeof(DATA_TYPE));
  }
  DATA_TYPE** y1;
  y1 = malloc(w * sizeof(DATA_TYPE*));
  for (int i = 0; i < w; i++) {
    y1[i] = malloc(h * sizeof(DATA_TYPE));
  }
  DATA_TYPE** y2;
  y2 = malloc(w * sizeof(DATA_TYPE*));
  for (int i = 0; i < w; i++) {
    y2[i] = malloc(h * sizeof(DATA_TYPE));
  }
  


  /* Initialize array(s). */
  init_array (w, h, &alpha, imgIn, imgOut);

  /* Start timer. */
  

  /* Run kernel. */
  kernel_deriche (w, h, alpha, imgIn, imgOut, y1, y2);

  /* Stop and print timer. */



  /* Prevent dead-code elimination. All live-out data must be printed
     by the function call in argument. */
  print_array(w, h, imgOut);

  /* Be clean. */
  for (int i = 0; i < w; i++) {
    free(imgIn[i]);
  }
  free(imgIn);

  for (int i = 0; i < w; i++) {
    free(imgOut[i]);
  }
  free(imgOut);

  for (int i = 0; i < w; i++) {
    free(y1[i]);
  }
  free(y1);

  for (int i = 0; i < w; i++) {
    free(y2[i]);
  }
  free(y2);

  return 0;
}
